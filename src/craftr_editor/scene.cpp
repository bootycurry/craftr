...
Scene::Scene() {
  ...
  scene_view.signal_unrealize().connect(
      sigc::mem_fun(*this, &Scene::on_unrealize));
  scene_view.signal_render().connect(sigc::mem_fun(*this, &Scene::on_render),
                                     false);
...
void Scene::ensure_gl_loaded() {
  ...
  if (!gladLoadGL()) {
    Logger::instance().log(LogDestination::LOG_WINDOW,
                           Glib::ustring("Failed to load GL via glad"),
                           LogType::ERROR);
    return;
  }

  // Simple square: two triangles, positions only (NDC, we'll scale/translate in
  // vertex shader)
  const float verts[] = {// x, y
...
void main() {
  ...
  if (!shader_program) {
    Logger::instance().log(LogDestination::LOG_WINDOW,
                           Glib::ustring("Failed to create shader program"),
                           LogType::ERROR);
  } else {
...
bool Scene::on_render(const Glib::RefPtr<Gdk::GLContext> &) {
  ...
  // For simplicity we'll use pixel-space mapping: world units ~ pixels with
  // center at (0,0) build projection matrix manually:
...
GLuint Scene::compile_shader(GLenum type, const char *src) {
  ...
  if (!status) {
    ...
    Logger::instance().log(LogDestination::LOG_WINDOW,
                           Glib::ustring("Shader compilation failed: " + log),
                           LogType::ERROR);
...
GLuint Scene::create_program(const char *vs_src, const char *fs_src) {
  ...
  if (!status) {
    ...
    Logger::instance().log(LogDestination::LOG_WINDOW,
                           Glib::ustring("Program link failed: " + log),
                           LogType::ERROR);
