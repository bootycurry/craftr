...
void InspectorPanel::refresh() {
  ...
  for (auto& [typeIdx, compPtr] : selected_object->get_all_components()) {
    if (dynamic_cast<Transform*>(compPtr.get())) continue;
    ...
    for (auto& prop : component->get_properties()) {
        ...
        switch (prop.type) {
            case PropertyType::INT: {
                auto adj = Gtk::Adjustment::create(std::stoi(prop.getter()), -100000, 100000, 1);
            ...
            case PropertyType::FLOAT: {
                auto adj = Gtk::Adjustment::create(std::stof(prop.getter()), -100000.0, 100000.0, 0.1);
            ...
            case PropertyType::BOOL: {
                ...
                check->signal_toggled().connect([&prop, check]() {
                    prop.setter(check->get_active() ? "1" : "0");
                });
                hbox->append(*check);
                break;
            }
            ...
            default: {
                ...
                entry->signal_changed().connect([&prop, entry]() {
                    prop.setter(entry->get_text());
                });
                hbox->append(*entry);
                break;
            }
...
void InspectorPanel::show_add_component_menu() {
  ...
  for (const auto& entry : ComponentRegistry::instance().get_all()) {
    ...
    // Skip Transform
    if (typeIdx == std::type_index(typeid(Transform))) continue;
    ...
    btn->signal_clicked().connect([this, typeIdx]() {
        ...
        try {
            ...
            Logger::instance().log(LogDestination::LOG_WINDOW, e.what(), LogType::ERROR);
        }
    });
